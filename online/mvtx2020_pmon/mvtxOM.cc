...
std::map < pair<int, int>, pair<int, int>> chip_map = {
  {{1,1}, {0,2}},
  {{1,2}, {0,1}},
  {{1,3}, {0,0}},
  {{1,4}, {0,3}},
      ...
      for ( int ruid=0; ruid < IDMVTXV2_MAXRUID+1; ruid++ )
      {
        if ( p->iValue(ruid) != -1 ) //(_lanes_active info) if -1 no information for RU ruid found in the event
        {
          for ( int ruchn = 0; ruchn < IDMVTXV2_MAXRUCHN+1; ruchn++ )
          {
            ...
            {
              int chipId_from_decoder = p->iValue(ruid, ruchn, "CHIP_ID");
              for ( int i = 0; i < p->iValue(ruid, ruchn); i++ )
              {
                int hit = p->iValue(ruid, ruchn, i); // Get hit
                int irow = decode_row(hit);
                int icol = decode_col(hit);
                //cout << "(ruid " << ruid << ", ruchn " << ruchn << ") ";
                //cout << "(row " << irow << ", col " << icol << ") ";
                if (chip_map.count({ruid,ruchn}) != 1)
                {
                  cout << "invalid: (ruid " << ruid << ", ruchn " << ruchn << ") " << endl;
                }
                else
                {
                  std::pair<int, int> chip_location = chip_map[ {ruid, ruchn} ];
                  int istave = chip_location.first;
                  int ichip  = chip_location.second;
                  if ( ichip != chipId_from_decoder )
                  {
                    std::cout << "WARNING: " << "Mismatch in chipId from readout chipId (" << chipId_from_decoder << ") and RU channel mapping (" << ichip << ") " << std::endl;
                  }
                  npixels[istave]++;
                  mrow[istave]+=irow;
